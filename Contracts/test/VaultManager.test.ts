...
describe("VaultManager", function () {
  ...
  beforeEach(async function () {
    ...
    if (
      "waitForDeployment" in oracle &&
      typeof oracle.waitForDeployment === "function"
    ) {
      await oracle.waitForDeployment();
    }

    const Vault = await ethers.getContractFactory("VaultManager");
    vault = await Vault.deploy(await oracle.getAddress(), owner.address); // Add treasury address
    if (
      "waitForDeployment" in vault &&
      typeof vault.waitForDeployment === "function"
    ) {
    ...
    if (
      "waitForDeployment" in weth &&
      typeof weth.waitForDeployment === "function"
    ) {
      await weth.waitForDeployment();
    }
    usdt = await Token.deploy("Tether USD", "USDT");
    if (
      "waitForDeployment" in usdt &&
      typeof usdt.waitForDeployment === "function"
    ) {
    ...
    await oracle.emergencySetPrice(wethAddr, ethers.parseEther("2000")); // $2000
    await oracle.emergencySetPrice(usdtAddr, ethers.parseEther("1")); // $1

    await vault.addCollateral(wethAddr);
    await vault.addBorrowToken(usdtAddr);
  ...
  it("Should borrow against collateral", async function () {
    ...
    const tx = await vault
      .connect(user1)
      .borrow(wethAddr, usdtAddr, borrowAmount);
    await expect(tx)
  ...
  it("Should repay debt", async function () {
    ...
    const lastUpdate = await vault.lastInterestUpdate(user1.address, usdtAddr);
